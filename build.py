      ...
      <div id="checkout-modal-body" class="checkout-body no-scrollbar">
        <form id="checkout-form" novalidate>
          ...
          <div id="checkout-step2" style="display:none;">
            ...
            <!-- TEMPORARY: mirror of the note in index.html. Delete both together
                 once the acquirer enables Amex/Diners, then rerun build.py. -->
            <p class="checkout-note" style="border-top:1px solid var(--sand-300);padding-top:1rem;">לתשומת לבכם: כרגע איננו מקבלים כרטיסי <span style="color:var(--ink);">אמריקן אקספרס ודיינרס</span> — גם לא בתשלום דרך Apple Pay.&lrm;</p>
...
SCRIPTS = '''  <script src="/js/nav.js?v=1"></script>
  ...
  <script src="/js/checkout.js?v=13"></script>
  ...
  (function(){
    ...
    if (status.startsWith('success')) {
      // Before clearCart(): consuming the pending-purchase record here is what
      // stops a refresh of this URL reporting a second Purchase.
      if (typeof stlTrackPurchaseFromRedirect === 'function') stlTrackPurchaseFromRedirect(p.get('order_id'));
    ...
    } else { if (typeof stlClearPendingPurchase === 'function') stlClearPendingPurchase(); icon.innerHTML='<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>'; title.textContent='התשלום לא הושלם'; msg.textContent='הכרטיס לא חויב. אפשר לנסות שוב או ליצור איתנו קשר.'; }
    modal.style.display='flex';
  })();
  </script>
'''

# Meta Pixel base snippet, generated by server.py from the META_PIXEL_ID env
# var. Kept out of the committed HTML on purpose: build.py runs on a laptop, so
# a build-time env var could never reach the Railway deploy.
META_PIXEL_TAG = '<script src="/meta/pixel.js"></script>'
...
{MODALS}
{SCRIPTS}
  <!-- Meta ViewContent. The value is the price actually asked for, so a
       discounted item doesn't report its pre-discount figure. -->
  <script>
    window.STEELO_PRODUCT = {json.dumps({
        "id": pid, "name": name, "category": cat,
        "price": price - round(price * (p.get("discount") or 0) / 100),
        "currency": "ILS",
    }, ensure_ascii=False)};
    if (typeof stlTrackViewContent === 'function') stlTrackViewContent();
    /* Fired here rather than inside analytics.js: that file runs with the other
       scripts, before this block defines STEELO_PRODUCT. */
    if (typeof stlTrack === 'function') stlTrack('view_product');
